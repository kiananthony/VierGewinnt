...
## Theorie Design Patterns - Mostafa
Prozeduraler / Modularer Aufbau.  Der Code folgt einem prozeduralen Stil ohne Klassen. Logisch zusammengehörige Operationen sind in klar benannte Funktionen gegliedert (z. B. finde_tiefste_zeile, spalte_ist_gueltig, stein_setzen), was Lesbarkeit und gezielte Testbarkeit ermöglicht.
Globaler Zustand (Shared Mutable State).  spielbrett und quads_indices sind globale Dictionaries, die von allen Funktionen direkt gelesen und beschrieben werden. Das vereinfacht den Einstieg, koppelt die Funktionen jedoch eng an den globalen Zustand und erschwert parallele Nutzung oder mehrere Spielinstanzen.
Precomputed Lookup Table.  quads_bestimmen() berechnet einmalig alle möglichen Vierergruppen (Quads) und speichert über quads_indices, welche Quads jede Zelle berührt. Dadurch reduziert stein_setzen() die Gewinnprüfung von O(n⁴) auf O(1) pro Zug – ein klassisches Space-for-Time-Tradeoff.
Datengesteuertes Design (Data-Driven Design).  Die acht Richtungsvektoren in RICHTUNGEN steuern sowohl die Gewinnprüfung als auch die Quad-Generierung. Neue Richtungen lassen sich rein durch Datenerweiterung ergänzen, ohne Algorithmen anzupassen – ein einfaches Beispiel des Open/Closed-Prinzips.
Sparse Representation.  Das Spielbrett wird nicht als 2D-Array, sondern als Dictionary mit (spalte, zeile)-Tupeln als Schlüssel gespeichert. Leere Felder existieren schlicht nicht als Einträge, was Speicher spart und Belegungsprüfungen zu einfachen in-Abfragen macht.
Testbarkeit durch Fixtures.  Die pytest-Tests nutzen autouse-Fixtures, um globalen Zustand vor jedem Test zurückzusetzen. Das gleicht das Fehlen von Kapselung aus und stellt sicher, dass Tests isoliert und reproduzierbar ablaufen.
